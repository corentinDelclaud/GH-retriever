...
def get_accuracy_expla_graphs(path):
    ...
    for pred, label in zip(df["pred"].tolist(), df["label"].tolist()):
        # For expla_graphs, treat each prediction as a single string
        prediction = pred.replace("|", "\n").split("\n")
        answer = [label]
        # Use eval_f1 for consistency with webqsp
        _, precision_score, recall_score = eval_f1(prediction, answer)
        precision_list.append(precision_score)
        recall_list.append(recall_score)
        acc = eval_acc(" ".join(prediction), answer)
        acc_list.append(acc)
